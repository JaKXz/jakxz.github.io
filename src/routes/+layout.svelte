...
{#key data.path}
	<main
		...
		class={classNames(
			...
			data.path !== '/' &&
				'relative xs:shadow-xl mt-[-1rem] bg-white dark:bg-[var(--dark)] border-solid border-0.1 border-b-0 xs:border-x-1 xs:border-t-1 xs:border-rounded-t-lg border-gray px-8 py-10 xs:px-24 xs:py-16 z-2 xs:max-w-60rem'
		)}
	>
		{@render children?.()}
	</main>
